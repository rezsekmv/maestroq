...
| `devices[].gpu`                  | `host` \| `swiftshader_indirect` \| `auto` | `host`                  | Android only. GPU mode for a headless cold-boot's `emulator -gpu <mode>`. Unset defaults to `host` (with a one-time automatic fallback to `swiftshader_indirect` if the host-GPU boot fails). `-no-window` otherwise silently picks software rendering, which makes GPU-heavy RN/Flutter apps miss Maestro's first-tap deadline. Pin a value to disable the fallback. Ignored for non-headless boots (they pick up the AVD's `hw.gpu.mode`). |
