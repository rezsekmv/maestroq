...
| `devices[].headless`             | boolean              | `false`                                | Android: when the daemon cold-boots the emulator, append `-no-window -no-audio -no-boot-anim` plus a `-gpu` flag (see `devices[].gpu`). No effect if the emulator is already running. iOS: declarative only — `simctl boot` is already headless, but `Simulator.app` attaches a window to every booted sim if it's open. The daemon does not quit it for you; close it manually (`osascript -e 'quit app "Simulator"'`) to run headlessly. |
| `devices[].gpu`                  | `host` \| `swiftshader_indirect` \| `auto` | `swiftshader_indirect`  | Android only. GPU mode for a headless cold-boot's `emulator -gpu <mode>`. Defaults to `swiftshader_indirect`: on Apple Silicon `-gpu host` translates GL→Metal and a GPU-heavy RN/Flutter app's first-frame render starves SystemUI → a "System UI isn't responding" ANR that overlays the app and eats every tap (the whole suite fails on the first interaction). Software rendering avoids it. Set `host` only on hosts with a real GL stack (x86 Linux) where it's stable and faster. Ignored for non-headless boots (they pick up the AVD's `hw.gpu.mode`). |
