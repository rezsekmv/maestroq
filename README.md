...
| `devices[].headless`             | boolean              | `false`                                | Android: when the daemon cold-boots the emulator, append `-no-window -no-audio -no-boot-anim`. No effect if the emulator is already running. iOS: `simctl boot` is already headless, but `Simulator.app` attaches a window to every booted sim if it's open. When `headless: true`, the daemon quits a running `Simulator.app` before each boot so no window attaches (logged). |
